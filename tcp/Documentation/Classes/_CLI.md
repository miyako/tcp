...
| controller | 4D.Class | -> | Controller class to attach (default: `cs.tcp._CLI_Controller`) |

## Description

`_CLI` resolves the platform-specific path to a bundled executable and attaches a controller that manages the underlying `4D.SystemWorker`. It is extended by [`tcp`](tcp.md) and should not be instantiated directly.
...
| controller | cs.tcp._CLI_Controller | Attached controller instance |
...
- [`tcp`](tcp.md) — extends `_CLI` to wrap the `procs` executable
