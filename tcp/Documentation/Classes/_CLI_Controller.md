...
> _CLI_Controller.new (CLI : cs.tcp._CLI)
...
| instance | cs.tcp._CLI | The owning `_CLI` instance |
...
#### execute (command, message, context) → cs.tcp._CLI_Controller
...
| Result | cs.tcp._CLI_Controller | <- | `This` — enables chaining |
...
- [`_tcp_Controller`](_tcp_Controller.md) — extends `_CLI_Controller` with stdout/stderr accumulation
